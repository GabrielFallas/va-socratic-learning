...
\begin{frame}{Metodología}
  \begin{itemize}
    \item \textbf{Tareas:} (T1) bucle infinito; (T2) algoritmo $O(n^2)\!\to\!O(n)$. Verificación
    ...
    \item \textbf{Estudio piloto:} $N=6$ entre-sujetos (3+3), asignación contrabalanceada.
    \item \textbf{Diseño refinado} (ya implementado, para el estudio completo): \emph{crossover}
          intra-sujeto con \textbf{variantes de tarea} (sin repetir el problema idéntico).
  \end{itemize}
\end{frame}

\begin{frame}{Resultados del piloto ($n=3$ por condición)}
  \centering
  \footnotesize
  \begin{tabular}{@{}lccc@{}}
    \toprule
    ...
    \midrule
    Godspeed global (1--5)   & \textbf{4,37} & 2,43 & \textbf{+2,0} \\
    NASA-TLX (carga)         & \textbf{42,6} & 72,8 & \textbf{$-$1,5} \\
    SUS (usabilidad)         & 66,7 & 65,8 & 0,05 \\
    Resolución autónoma      & 0,33 & 0,50 & $-$0,3 \\
    Turnos por tarea         & 3,3 & 9,3 & $-$1,4 \\
    PANAS afecto $+$         & 18,3 & 15,7 & 0,5 \\
    \% latencia $<1.5$\,s    & 94 & 95 & --- \\
    \bottomrule
  \end{tabular}

  \vspace{0.6em}
  \footnotesize Con $N$ pequeño se priorizan \textbf{tamaños de efecto}, no $p$.
\end{frame}

\begin{frame}{Lo que dicen los datos}
  \begin{itemize}
    \item \textbf{RQ1:} corporización $\Rightarrow$ percepción del agente \alert{mucho mayor}
          ($d=2{,}0$) \emph{sin} mejorar usabilidad $\Rightarrow$ efecto de \textbf{presencia
          social}.
    \item \textbf{RQ3:} carga cognitiva autopercibida \alert{menor} en A ($d=-1{,}5$); la
          corporización \emph{no} aumentó la carga en una tarea densa.
    \item \textbf{RQ2:} resolución similar; A usó menos turnos/tiempo (confundido con experiencia
          previa).
    \item \textbf{RQ4:} 94\,\% de respuestas $<1.5$~s con inferencia local. \checkmark
  \end{itemize}
  \begin{block}{Hallazgo cualitativo más claro}
    Los participantes de \textbf{texto pidieron el avatar}: \emph{``No me gustó no ver a Sonic''};
    \emph{``Prefiero aprender junto con un agente virtual como Sonic''}.
  \end{block}
\end{frame}

\begin{frame}{Limitaciones (honestas)}
  \begin{itemize}
    \item $n=3$ por condición: efectos grandes pero \textbf{inestables}; sin significancia.
    \item Diseño entre-sujetos: experiencia previa no balanceada $\Rightarrow$ RQ2 no causal.
    \item Muestra homogénea (hombres jóvenes); posible \textbf{efecto de novedad} del personaje.
    \item Afecto \emph{post-only}; latencia dependiente del hardware local.
  \end{itemize}
  \vspace{0.4em}
