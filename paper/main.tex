...
\title{Sonic Code: un tutor socr\'atico multimodal corporizado para el desarrollo de l\'ogica de programaci\'on en entornos STEM}
...
presenta \emph{Sonic Code}, un tutor socr\'atico que gu\'ia al estudiante mediante preguntas
...
\begin{table}[t]
  \caption{Conjunto de instrumentos}
...
\begin{figure}[t]
  ...
  \begin{tikzpicture}[
    ...
    step/.style={draw, rounded corners=2pt, fill=blue!7, align=center, font=\scriptsize,
                 inner sep=2pt, text width=12mm, minimum height=7mm},
    store/.style={draw, fill=green!10, align=center, font=\scriptsize, inner sep=2pt,
                  text width=12mm, minimum height=7mm},
    >={Stealth[length=1.4mm]}]
    \node[step] (assign) {Asignaci\'on};
    \node[step, below=of assign] (inter) {Interacci\'on};
    \node[step, below=of inter] (quest) {Cuestionarios};
    \node[store, below=of quest] (db) {SQLite};
    \node[store, below=of db] (export) {Export};
    \node[step, below=of export] (analysis) {An\'alisis};
    \draw[->] (assign) -- (inter);
    \draw[->] (inter) -- (quest);
    \draw[->] (quest) -- (db);
    \draw[->] (db) -- (export);
...
El an\'alisis inferencial (Mann--Whitney $U$, $\delta$ de Cliff, $d$ de Cohen) se
...
Se present\'o \emph{Sonic Code}, un tutor socr\'atico multimodal corporizado, completamente local y
