...
2. **NUNCA uses los botones `Prueba A` / `Prueba B`** con participantes reales: generan IDs
   `P-PILOT-*` que se **excluyen** del análisis (son solo para pruebas técnicas).
...
- En `/admin`, confirma que apareció una fila nueva **Real** con la condición esperada y que el
  contador de cuestionarios es 6 (batería completa).
